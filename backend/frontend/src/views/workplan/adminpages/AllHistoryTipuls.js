...
const AllHistoryTipuls = ({ match }) => {
  ...
  const loadOgdas = async () => {
    let temppikodogdas = [];
    for (let i = 0; i < alltipulshistoryfilters.pikods.length; i++) {
      await axios.post("http://localhost:8000/api/ogda/ogdasbypikodid", { pikod: alltipulshistoryfilters.pikods[i] })
        .then(response => {
  ...
  useEffect(() => {
    setOgdas([]);
    setAlltipulshistoryfilters({ ...alltipulshistoryfilters, ogdas: [] });
    loadOgdas();
  }, [alltipulshistoryfilters.pikods]);
      ...
      <Card>
        <CardBody>
          ...
          <Collapse isOpen={isOpen}>
            <UnitSelect pagefilter={alltipulshistoryfilters} pikods={pikods} ogdas={ogdas} hativas={hativas} gdods={gdods} handleChange2={handleChange2} level={"admin"} />
          ...
          <SortingTable
            gdodsid={alltipulshistoryfilters.gdods} hativasid={alltipulshistoryfilters.hativas} ogdasid={alltipulshistoryfilters.ogdas} pikodsid={alltipulshistoryfilters.pikods}
...
export default withRouter(AllHistoryTipuls);;