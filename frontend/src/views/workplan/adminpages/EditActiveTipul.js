...
const EditActiveTipul = ({ match }) => {
  ...
  const loadTipultype = () => {
    axios
      .get("http://localhost:8000/api/tipultype")
      .then((response) => {
  ...
  const loadZakaottipul = () => {
    axios
      .get("http://localhost:8000/api/zkaottipul")
      .then((response) => {
  ...
  const loadGofbizoa = () => {
    axios
      .get("http://localhost:8000/api/gofbizoa")
      .then((response) => {
  ...
  async function UpdateActiveTipul() {
    //need to add logging
    
    var tempactivetipul = tipuldata;
    var temphistorytipul = historytipuldata;
    console.log('datasent',tempactivetipul)

    temphistorytipul.originaltipulid = tipuldata.originaltipulid;
    ...
    await axios
      .post(`http://localhost:8000/api/historytipul`, {historytipuldata})
      .then((res) => {
    ...
    await axios
      .post(
        `http://localhost:8000/api/activetipul/update/${match.params.tipulid}`,
        { tempactivetipul }
      )
      .then((res) => {
  ...
  function init() {
    var activetipulidtemp = match.params.tipulid;
    axios
      .get(`http://localhost:8000/api/activetipulbyid/${activetipulidtemp}`) //need to make a new backend route
      .then((res) => {
        ...
        temptipul.carteam =  temptipul.carteam[0]
        temptipul.gdodbizoa =  temptipul.gdodbizoa[0]
        temptipul.sadnabizoa =  temptipul.sadnabizoa[0]
        setTipulData(temptipul);
        setHistoryTipulData(temptipul);
      })
      .catch((error) => {});
  }
  const currcar = tipuldata.carteam
  useEffect(() => {
        ...
        <CardBody>
            ...
            <>
                ...
                <Col xs={12} md={3}>
                  ...
                  <FormGroup dir="rtl">
                    <Input
                      type="select"
                      name="pikod"
                      value={tipuldata.car.gdod.hativa.ogda.pikod._id}
                      onChange={handleChange}
                      disabled
                    >
                ...
                <Col xs={12} md={3}>
                  ...
                  <FormGroup dir="rtl">
                    <Input
                      type="select"
                      name="ogda"
                      value={tipuldata.car.gdod.hativa.ogda._id}
                      onChange={handleChange}
                      disabled
                    >
                ...
                <Col xs={12} md={3}>
                  ...
                  <FormGroup dir="rtl">
                    <Input
                      type="select"
                      name="hativa"
                      value={tipuldata.car.gdod.hativa._id}
                      onChange={handleChange}
                      disabled
                    >
              ...
              <FormGroup dir="rtl">
                <Input
                  ...
                  bsSize="lg"
                  name="magadal"
                  value={tipuldata.car.mkat.mkabaz.magad.magadal._id}
                  onChange={handleChange}
                  disabled
                >
              ...
              <FormGroup dir="rtl">
                <Input
                  ...
                  bsSize="lg"
                  name="magad"
                  value={tipuldata.car.mkat.mkabaz.magad._id}
                  onChange={handleChange}
                  disabled
                >
              ...
              <FormGroup dir="rtl">
                <Input
                  ...
                  bsSize="lg"
                  name="mkabaz"
                  value={tipuldata.car.mkat.mkabaz._id}
                  onChange={handleChange}
                  disabled
                >
          ...
          <FormGroup dir="rtl">
            <Input
              type="date"
              name="lastipuldate"
              value={tipuldata.lastipuldate}
              onChange={handleChange}
              onBlur={() =>
                simpleValidator.current.showMessageFor("lastipuldate")
              }
            />
            {simpleValidator.current.message(
              "lastipuldate",
              tipuldata.lastipuldate,
              "required"
            )}
          </FormGroup>
            ...
            <>
              ...
              <FormGroup dir="rtl">
                <Input
                  ...
                  bsSize="lg"
                  name="zkaottipul"
                  value={tipuldata.zkaottipul._id}
                  onChange={handleChange}
                >
          ...
          <FormGroup dir="rtl">
            <Input
              type="date"
              name="tolarancedate"
              value={tipuldata.tolarancedate}
              onChange={handleChange}
              onBlur={() =>
                simpleValidator.current.showMessageFor("tolarancedate")
              }
            />
            {simpleValidator.current.message(
              "tolarancedate",
              tipuldata.tolarancedate,
              "required"
            )}
          </FormGroup>
          ...
          <>
            {tipuldata.gdodbizoa ? (
              <>
              {console.log(tipuldata.gdodbizoa)}
                <div
                  style={{
                    textAlign: "right",
                    paddingTop: "10px",
          ...
          <>
            {tipuldata.sadnabizoa ? (
              <>
              {console.log(tipuldata.sadnabizoa)}
                <div
                  style={{
                    textAlign: "right",
                    paddingTop: "10px",
                ...
                <FormGroup dir="rtl">
                  <Input
                    ...
                    bsSize="lg"
                    name="sadnabizoa"
                    value={tipuldata.sadnabizoa._id}
                    onChange={handleChange}
                  >
      ...
      {/* <Card>
          ...
          {tipuldata.sadnabizoa ?
            ...
            : <>
              <div style={{ textAlign: 'right', paddingTop: '10px', fontWeight: "bold" }}>סדנא לביצוע</div>
              <FormGroup dir="rtl" >
                <Input type="select" bsSize="lg" name="sadnabizoa" onChange={handleChange}>
                  <option value="">בחר סדנא</option>
                  {sadnabizoa.map((data, index) => (
                    <option value={data._id}>{data.name}</option>
                  ))}
...
export default withRouter(EditActiveTipul);
