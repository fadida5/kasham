...
function AdminUpkeepPlan() {
  ...
  const loadOgdas = async () => {
    let temppikodogdas = [];
    for (let i = 0; i < magadalcarddata.pikods.length; i++) {
      await axios.post("http://localhost:8000/api/ogda/ogdasbypikodid", { pikod: magadalcarddata.pikods[i] })
        .then(response => {
  ...
  useEffect(() => {
    setOgdas([]);
    setMagadalcarddata({ ...magadalcarddata, ogdas: [] });
    loadOgdas();
  }, [magadalcarddata.pikods]);
    ...
    <>
      <Card>
        <CardBody>
          ...
          <UnitSelect pagefilter={magadalcarddata} pikods={pikods} ogdas={ogdas} hativas={hativas} gdods={gdods} handleChange2={handleChange2} level={"admin"}/>
            ...
            {magadals.map((magadal, i) => (
              magadal ?
                <>
                  <Col xs={12} md={3}>
                    <MeagedalCard 
                    gdodsid={magadalcarddata.gdods} hativasid={magadalcarddata.hativas} ogdasid={magadalcarddata.ogdas} pikodsid={magadalcarddata.pikods}
...
export default AdminUpkeepPlan;