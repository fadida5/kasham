...
const AdminTable = ({ match }) => {
  ...
  const loadOgdas = async () => {
    let temppikodogdas = [];
    for (let i = 0; i < tipulsfilters.pikods.length; i++) {
      await axios.post("http://localhost:8000/api/ogda/ogdasbypikodid", { pikod: tipulsfilters.pikods[i] })
        .then(response => {
          for (let j = 0; j < response.data.length; j++)
  ...
  const loadHativas = async () => {
    let tempogdahativas = [];
    for (let i = 0; i < tipulsfilters.ogdas.length; i++) {
      await axios.post("http://localhost:8000/api/hativa/hativasbyogdaid", { ogda: tipulsfilters.ogdas[i] })
        .then(response => {
          for (let j = 0; j < response.data.length; j++)
  ...
  const loadGdods = async () => {
    ...
    for (let i = 0; i < tipulsfilters.hativas.length; i++) {
      await axios.post("http://localhost:8000/api/gdod/gdodsbyhativaid", { hativa: tipulsfilters.hativas[i] })
        .then(response => {
          for (let j = 0; j < response.data.length; j++)
  ...
  useEffect(() => {
    setOgdas([]);
    setTipulsfilters({ ...tipulsfilters, ogdas: [] });
    loadOgdas();
  }, [tipulsfilters.pikods]);

  useEffect(() => {
    setHativas([]);
    setTipulsfilters({ ...tipulsfilters, hativas: [] });
    loadHativas();
  }, [tipulsfilters.ogdas]);

  useEffect(() => {
    setGdods([]);
    setTipulsfilters({ ...tipulsfilters, gdods: [] });
    loadGdods();
  }, [tipulsfilters.hativas]);
    ...
    <Card>
      <CardBody>
        ...
        <Collapse isOpen={isOpen}>
          <UnitSelect pagefilter={tipulsfilters} pikods={pikods} ogdas={ogdas} hativas={hativas} gdods={gdods} handleChange2={handleChange2} level={"admin"} />
        ...
        <SortingTable
          gdodsid={tipulsfilters.gdods} hativasid={tipulsfilters.hativas} ogdasid={tipulsfilters.ogdas} pikodsid={tipulsfilters.pikods}
