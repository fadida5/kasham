...
const EditMatagForm = ({ match }) => {
  ...
  function UpdateMatag() {
    var tempmatag = matagdata;
    delete tempmatag._id; //cant send _id 
    delete tempmatag.createdAt; //fix the date
    delete tempmatag.updatedAt; //fix the date  
  ...
  function init() {
    var matagidtemp = match.params.matagid;
    setTempHativaId(match.params.hativaid)
    axios.get(`http://localhost:8000/api/matag/${matagidtemp}`)
      .then(res => {
        setMatagData(res.data[0]);
      })
      .catch(error => {
            ...
            <CardBody>
              <Container>
                ...
                <div style={{ textAlign: 'center' }}>
                  <button className="btn" onClick={clickSubmit}>עדכן אימון חטיבתי</button>
                </div>
...
export default withRouter(EditMatagForm);;