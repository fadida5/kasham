...
const EditKshirotOnly = ({ match }) => {
  ...
  function checkspecialkeys() {
    ...
    if (finalspecialkey.length == 0) {
      ...
      for (let i = 0; i < finalspecialkey.length; i++) {
        if (alreadyerrored == false) {
          if ((finalspecialkey[i].matzva < 0) || (finalspecialkey[i].name == "") || (finalspecialkey[i].teken < 0)) {
            IsValidTipul = false;
            alreadyerrored = true;
            ErrorReason += 'תקלה ברשימת הקצינים, אחד או יותר מהשדות ריק או לא תקין \n'
          }
        }
      }
    }
    ...
    if (finalspecialkeytwo.length == 0) {
      ...
      for (let i = 0; i < finalspecialkeytwo.length; i++) {
        if (alreadyerrored == false) {
          if ((finalspecialkeytwo[i].matzva < 0) || (finalspecialkeytwo[i].name == "") || (finalspecialkeytwo[i].teken < 0)) {
            IsValidTipul = false;
            alreadyerrored = true;
            ErrorReason += 'תקלה ברשימת בעלי תפקיד, אחד או יותר מהשדות ריק או לא תקין \n'
          }
        }
      }
    }
  ...
  function UpdateKshirot() {
    ...
    axios.post(`http://localhost:8000/api/kshirot/updatekshirot`, [tipulidtemp, temptipul])
      .then(res => {
  ...
  function init() {
    var tipulidtemp = match.params.tipulid;
    axios.get(`http://localhost:8000/api/kshirot/${tipulidtemp}`)
      .then(res => {
  ...
  useEffect(() => { //calculate officers
    ...
    setTipulData({ ...tipuldata, officers: tempofficers, officersmax: tempofficersmax, specialkey: finalspecialkey});
  }, [finalspecialkey])
  ...
  useEffect(() => { //calculate kzinim
    ...
    setTipulData({ ...tipuldata, kzinim: tempkzinim, kzinimmax: tempkzinimmax , specialkeytwo: finalspecialkeytwo});
  }, [finalspecialkeytwo])
        ...
        <Col xs={12} md={6}>
          <Card>
            <CardHeader style={{ direction: 'rtl' }}>
              <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'center', fontWeight: "bold" }}>הערכת כשירות טנא</CardTitle>{/*headline*/}
            </CardHeader>
              ...
              <Container>
                ...
                <Row>
                  ...
                  <Col xs={12} md={6}>
                    ...
                    <FormGroup dir="rtl" >
                      <Input type="date" bsSize="lg" name="timeinrole" value={timeinrolevalue} onChange={handleChange} style={{lineHeight:'inherit'}}
                        onBlur={()=>simpleValidator.current.showMessageFor('timeinrole')} />
                      {simpleValidator.current.message('timeinrole', timeinrolevalue, 'required')}
                    </FormGroup>
                            ...
                            <Col xs={12} md={4}>
                              <div>
                                ...
                                <Input onChange={(e) => {
                                  ...
                                  value={p.tafkiddetails} type="text" placeholder="הערות" />
                              </div>
                            </Col>
                          </Row>
                  ...
                  <Col xs={12} md={6}>
                    ...
                    <FormGroup dir="rtl" >
                      <Input type="number" bsSize="lg" name="bakashmax" value={tipuldata.bakashmax} onChange={handleChange} 
                         ...
                         {simpleValidator.current.message('bakashmax', tipuldata.bakash, 'required')}
                    </FormGroup>
                  </Col>
                </Row>
                ...
                <FormGroup dir="rtl" >
                  <Input type="date" bsSize="lg" name="lastrefreshdate" value={lastrefreshdatevalue} onChange={handleChange} style={{ lineHeight: 'inherit' }} 
                   onBlur={()=>simpleValidator.current.showMessageFor('lastrefreshdate')}/>
                   {simpleValidator.current.message('lastrefreshdate', lastrefreshdatevalue, 'required')}
                </FormGroup>
          ...
          <Card>
            <CardBody>
              <Container>
                ...
                <div style={{ textAlign: 'right', paddingTop: '10px', fontWeight: "bold" }}>פקלי״ם</div>
                <Row>
                  <Col xs={12} md={6}>
                ...
                <FormGroup dir="rtl" >
                  <Input type="date" name="trainingamount" value={trainingamountvalue} onChange={handleChange} 
                     onBlur={()=>simpleValidator.current.showMessageFor('trainingamount')}/>
                     {simpleValidator.current.message('trainingamount', trainingamountvalue, 'required')}
                </FormGroup>
                ...
                <FormGroup dir="rtl" >
                  <Input type="number" name="trainingquality" value={tipuldata.trainingquality} onChange={handleChange}
                ...
                <FormGroup dir="rtl" >
                  <Input type="number" name="battaliondrillquality" value={tipuldata.battaliondrillquality} onChange={handleChange}
