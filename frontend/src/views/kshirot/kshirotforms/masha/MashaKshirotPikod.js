...
const MashaKshirotPikod = ({ match }) => {
  ...
  function init() {
    var gdodidtemp = match.params.pikodid;
  ...
  function UpdateGdodsKshirot(addedkshirotid) {
    axios.post(`http://localhost:8000/api/pikod/updatekshirot`, [tempgdodid, addedkshirotid])
      .then(res => {
  ...
  function UpdateGdodsHistory(addedkshirotid) {
    axios.post(`http://localhost:8000/api/pikod/updatehistory`, [tempgdodid, addedkshirotid])
      .then(res => {
            ...
            <CardBody>
              <Container>
                ...
                <div style={{ textAlign: 'center' }}>
                  <button className="btn btn-primary" onClick={clickSubmit}>הוסף כשירות</button>
                </div>
...
export default withRouter(MashaKshirotPikod);;