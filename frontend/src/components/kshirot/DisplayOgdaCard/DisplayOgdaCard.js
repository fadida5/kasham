...
const DisplayOgdaCard = ({ id }) => {
    const { user } = isAuthenticated();
    const [ogda, setOgda] = useState([]);


    const [ogdakshirot, setOgdaKshirot] = useState(0);
    const [ogdatraining, setOgdaTraining] = useState(0);
    const [ogdamatag, setOgdaMatag] = useState(0);
    ...
    function getOgda() {
        axios.post(`http://localhost:8000/api/ogda/ogdabyid`, [id])
            .then(response => {
                setOgda(response.data[0]);
                findOgdaData(response.data[0]._id);
            })
            .catch(error => {
    ...
    const findOgdaData = async (ogdaid) => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            tempogdamatag += await loadhativamatag(data[i].matag)
            }
            ...
            for (let j = 0; j < data1.length; j++) {
                ...
                tempogdakshirot += await loadgdodkshirot(data1[j].kshirot)
                }
                if(data1[j].training)
                {
                gdodswithtrainingindex += 1;
                tempogdatraining += await loadgdodtraining(data1[j].training)
                }
            }
        }
        if(gdodswithkshirotindex!=0)
        tempogdakshirot = tempogdakshirot / gdodswithkshirotindex

        if(gdodswithtrainingindex!=0)
        tempogdatraining = tempogdatraining / gdodswithtrainingindex

        if(hativaswithmatagindex!=0)
        tempogdamatag = tempogdamatag / hativaswithmatagindex

        setOgdaKshirot(tempogdakshirot)
        setOgdaTraining(tempogdatraining)
        setOgdaMatag(tempogdamatag)
    };
    ...
    return (
        <>
            {id != null ?
                <>
                <Link style={{ color: "black" }} to={`/userogdaeditpage/${ogda._id}`}>
                    <Card>
                        <CardHeader style={{ direction: 'rtl' }}>
                            <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'right' }}>{ogda.name}</CardTitle>
                        </CardHeader>
                        ...
                        <CardBody style={{ direction: 'ltr', textAlign: 'right' }}>
                            <Row style={{ direction: 'rtl', textAlign: 'center' }}>
                                ...
                                <Col xs={12} md={4}>
                                    <h5 style={{ color: '#A7C7E7' }}>&#9632; ציון מטא"ג אוגדתי</h5>
                                </Col>
                            </Row>
                            ...
                            {ogdakshirot != 'NaN' ?
                                <h4 style={{ color: "#56758f", margin: '0px' }}>{ogdakshirot.toFixed(2)}:ציון כשירות גדודים באוגדה</h4>
                                : <h4 style={{ color: "#56758f", margin: '0px' }}>:ציון כשירות גדודים באוגדה</h4>
                            }
                            {ogdatraining != 'NaN' ?
                                <h4 style={{ color: "#92a9bd", margin: '0px' }}>{ogdatraining.toFixed(2)}:ציון אימוני גדוד באוגדה</h4>
                                : <h4 style={{ color: "#92a9bd", margin: '0px' }}>:ציון אימוני גדוד באוגדה</h4>
                            }
                            <h4 style={{ color: "#A7C7E7", margin: '0px' }}>{ogdamatag.toFixed(2)}:ציון מטא"ג אוגדתי</h4>
                        </CardBody>
                    </Card>
                    </Link>
                </>
...
export default withRouter(DisplayOgdaCard);;
